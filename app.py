...
from computer_use_agent.llm_env import agent_run_max_seconds
...
def _runtime_env_detail(*, check_playwright: bool = True) -> str | None:
    ...
    if not check_playwright:
        return None
    try:
        from computer_use_agent.browser.playwright_ctx import playwright_chromium_status
...
_computer_use_agent_agent = None
...
def _get_computer_use_agent_agent():
    """Load agent (GenAI + MCP / DAG) only when needed."""
    global _computer_use_agent_agent
    if _computer_use_agent_agent is not None:
        return _computer_use_agent_agent
    with _agent_lock:
        if _computer_use_agent_agent is None:
            if _agent_mode() == "loop":
                from computer_use_agent.agent import ComputerUseAgent

                _computer_use_agent_agent = ComputerUseAgent()
            else:
                from computer_use_agent.flow import DagAgent

                _computer_use_agent_agent = DagAgent()
        return _computer_use_agent_agent
...
async def lifespan(app: FastAPI):
    ...
    log_queue = None
    agent = _computer_use_agent_agent
...
app = FastAPI(title="Computer Use Agent", lifespan=lifespan)
...
async def _stop_agent_run(*, log_user_stop: bool = True) -> dict[str, Any]:
    ...
    if _agent_mode() == "dag":
        try:
            from computer_use_agent.graph_viz import latest_dag_session_id
            from computer_use_agent.persistence import SessionStore
...
async def run_agent(request: QueryRequest):
    ...
    async def _job():
        try:
            agent = _get_computer_use_agent_agent()
            if _agent_mode() == "dag":
            ...
            try:
                from computer_use_agent.flow import log_final_answer
...
async def resume_agent(request: ResumeRequest):
    ...
    from computer_use_agent.graph_viz import graph_viz_payload, prepare_session_for_resume
    ...
    async def _job():
        try:
            await asyncio.wait_for(
                _get_computer_use_agent_agent().resume(sid),
                timeout=agent_run_max_seconds(),
            )
        except asyncio.TimeoutError:
            logger.error(
...
async def reset_state():
    ...
    session_files = list((state_dir / "sessions").rglob("*")) if (state_dir / "sessions").exists() else []
    if state_dir.exists():
        try:
            await asyncio.to_thread(shutil.rmtree, state_dir)
            await _reload_agent_memory_async()
            logger.warning("[UI] State cleared, including DAG sessions and trajectory files.")
            return {
                "status": "success",
                "detail": (
                    "Durable state cleared, including memory, FAISS index, DAG sessions, "
                    "trajectory directories, and local session files."
                ),
                "removed_session_files": len(session_files),
            }
        except Exception as e:
...
async def _reload_agent_memory_async() -> None:
    ...
    if agent is not None and hasattr(agent, "memory"):
        ...
        return
    from computer_use_agent.memory import MemoryManager
...
async def api_dag_sessions():
    ...
    try:
        from computer_use_agent.graph_viz import list_dag_sessions
...
async def api_dag_graph(session_id: str | None = None):
    ...
    try:
        from computer_use_agent.graph_viz import graph_viz_payload, latest_dag_session_id
        from computer_use_agent.persistence import SessionLoadError
...
async def api_dag_browser_screenshot(session_id: str, path: str):
    ...
    try:
        from computer_use_agent.browser.page_capture import resolve_screenshot_path
...
@app.get("/api/dag/computer-replay")
async def api_dag_computer_replay(session_id: str, node_id: str | None = None):
    """Computer trajectory evidence report (path, actions, trajectory_dir, files)."""
    try:
        from computer_use_agent.computer.replay import build_computer_replay_report
        from computer_use_agent.graph_viz import latest_dag_session_id
        ...
        logger.error(f"[UI] computer replay failed: {e}")
        return JSONResponse({"status": "error", "detail": str(e)}, status_code=500)


@app.get("/api/dag/computer-artifact")
async def api_dag_computer_artifact(trajectory_dir: str, path: str):
    """Serve a file from a recorded computer trajectory directory."""
    try:
        from computer_use_agent.computer.replay import resolve_trajectory_artifact

        full = resolve_trajectory_artifact(trajectory_dir, path)
        if not full:
            return JSONResponse({"status": "error", "detail": "Artifact not found"}, status_code=404)
        media = "application/octet-stream"
        lower = full.suffix.lower()
        if lower == ".png":
            media = "image/png"
        ...
        logger.error(f"[UI] computer artifact failed: {e}")
        return JSONResponse({"status": "error", "detail": str(e)}, status_code=500)


@app.get("/api/dag/computer-evidence")
async def api_dag_computer_evidence():
    """List computer-use runs with trajectory_dir summaries for the Evidence tab."""
    try:
        from computer_use_agent.computer.replay import list_computer_evidence_sessions

        rows = await asyncio.to_thread(list_computer_evidence_sessions)
        return {"status": "success", "runs": rows}
    except Exception as e:
        logger.error(f"[UI] computer evidence list failed: {e}")
        return JSONResponse({"status": "error", "detail": str(e)}, status_code=500)


@app.get("/api/queries/dag")
async def api_dag_queries():
    """Assignment queries for browser and computer-use demos."""
    try:
        from computer_use_agent.catalog import assignment_payload, validate_assignment_corpus
...
async def api_browser_queries():
    """Assignment queries for browser and computer-use demos."""
    try:
        from computer_use_agent.catalog import browser_queries_payload, validate_assignment_corpus
...
async def vision_endpoint(body: VisionRequest):
    ...
    try:
        from computer_use_agent.vision_api import decode_image_payload, vision_analyze
...
async def api_browser_playwright_status(probe: bool = False):
    ...
    try:
        from computer_use_agent.browser.playwright_ctx import (
...
async def health():
    ...
    if pw_ready is None:
        pw_ready, pw_err = await asyncio.to_thread(
            lambda: __import__(
                "computer_use_agent.browser.playwright_ctx",
