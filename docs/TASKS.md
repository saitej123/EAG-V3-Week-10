...
| Executor runs ready nodes in parallel | `asyncio.gather` in `computer_use_agent/flow.py` |
...
Critic tools: `validate_json_keys`, `count_syllables` in `computer_use_agent/mcp_server.py`.
...
**Orchestrator:** no changes to `computer_use_agent/flow.py` — generic `SkillRegistry` dispatch runs `prosody_analyst` like any other skill.
...
| Item | Location |
|------|----------|
| Browser skill | `computer_use_agent/browser/` — extract/deterministic/a11y/vision cascade |
...
| Replay report | `computer_use_agent/browser/replay.py` → `GET /api/dag/browser-replay` |
