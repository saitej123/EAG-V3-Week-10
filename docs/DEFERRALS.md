...
**Where it lives:** `computer_use_agent/memory.py` — vector-first `read()` with keyword overlap as fallback only when embeddings or the index are unavailable.
...
**Where it lives:** `computer_use_agent/indexing.py` — `_chunk_text()`, `index_document_path()`.
...
**Where it lives:** `computer_use_agent/memory.py` — `_load_faiss_from_disk()` on every `read()`.
...
**Where it lives:** `computer_use_agent/llm_env.py` — `gemini_embed_model()`, `try_embed_text()`.
