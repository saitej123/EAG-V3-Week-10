...
User goal
  ...
  → Browser skill (`computer_use_agent/browser/drivers/`)
...
New behaviour plugs in via `agent_config.yaml` + `computer_use_agent/browser/` only.
...
`computer_use_agent/browser/drivers/interaction.py` — **A11yDriver** and **SetOfMarksDriver**. Element enumeration: `drivers/elements.py`. Mark drawing: `drivers/marks.py`. Gemini: `drivers/gemini_client.py`.
