...
from computer_use_agent.dag_schemas import NodeSpec, NodeStatus, PlannerOutput
from computer_use_agent.flow import Graph
from computer_use_agent.persistence import SessionStore, node_filename
...
def test_critic_splice_yields_ready_critic_node():
    ...
    from computer_use_agent.dag_schemas import NodeState
...
def test_format_memory_hits_preview():
    from computer_use_agent.schemas import MemoryItem
    from computer_use_agent.skills import format_memory_hits
...
def test_session_store_atomic_roundtrip(tmp_path, monkeypatch):
    ...
    from computer_use_agent.dag_schemas import AgentResult
...
def test_resolve_inputs_falls_back_to_graph_skill_when_agent_name_missing():
    from computer_use_agent.dag_schemas import AgentResult
    from computer_use_agent.skills import resolve_inputs
...
def test_structured_extraction_from_browser_uses_required_row_keys():
    from computer_use_agent.skills import _structured_extraction_from_browser
...
def test_prime_memory_read_uses_empty_history_and_top_k(monkeypatch):
    ...
    from computer_use_agent.flow import Executor
    ...
    monkeypatch.setattr("computer_use_agent.flow.format_memory_hits", lambda hits: "")

    async def _sync_to_thread(fn, *a, **kw):
        return fn(*a, **kw)

    monkeypatch.setattr("computer_use_agent.flow.asyncio.to_thread", _sync_to_thread)
