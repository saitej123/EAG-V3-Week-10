...
def perception_prompt_text() -> str:
    ...
    spec = importlib.util.spec_from_file_location("perception", ROOT / "computer_use_agent" / "perception.py")
    assert spec and spec.loader
    mod = importlib.util.module_from_spec(spec)
    # Extract prompt template by reading source — observe() builds prompt inline.
    src = (ROOT / "computer_use_agent" / "perception.py").read_text(encoding="utf-8")
...
def check_flowchart_compliance() -> list[str]:
    ...
    errors: list[str] = []
    extract_src = (ROOT / "computer_use_agent" / "browser" / "extract.py").read_text(encoding="utf-8")
    skill_src = (ROOT / "computer_use_agent" / "browser" / "skill.py").read_text(encoding="utf-8")
    mcp_src = (ROOT / "computer_use_agent" / "mcp_server.py").read_text(encoding="utf-8")
...
def main() -> int:
    ...
    if (ROOT / "computer_use_agent" / "memory.py").is_file() and "_format_hits" in (
        (ROOT / "computer_use_agent" / "memory.py").read_text()
    ):
