...
(function (global) {
    ...
    DagGraphController.prototype.getNodeData = function (nodeId) {
        const nodes = (this.payload && this.payload.nodes) || [];
        return nodes.find(function (n) {
            return String(n.id) === String(nodeId);
        }) || null;
    };

    DagGraphController.prototype.selectNode = function (nodeId) {
        if (!this.container) return;
        this.selectedNodeId = nodeId || null;
        this.container.querySelectorAll('.dag-svg-node').forEach(function (el) {
            el.classList.toggle(
                'dag-svg-selected',
                !!nodeId && el.getAttribute('data-dag-id') === String(nodeId)
            );
        });
    };
    ...
    DagGraphController.prototype._updateNodesInPlace = function (payload) {
        ...
        nodes.forEach(function (n) {
            ...
            if (ctrl.selectedNodeId && String(ctrl.selectedNodeId) === String(n.id)) {
                el.classList.add('dag-svg-selected');
            } else {
                el.classList.remove('dag-svg-selected');
            }
            if (changed) {
