...
Playwright, Pillow, httpx, trafilatura — **direct Gemini SDK** for a11y (text) and vision (VLM). `GEMINI_API_KEY` is required. **Not** crawl4ai (Researcher only).

Implementation: `computer_use_agent/browser/drivers/` (A11yDriver + SetOfMarksDriver).
