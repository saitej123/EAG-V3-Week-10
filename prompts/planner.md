...
Available skills:
  ...
  computer           native desktop computer-use via cua-driver (cascade: read → hotkey → electron page → AX → vision)
...
For native OS tasks, Electron desktop apps, or canvas-only desktop targets, route through
**computer** then formatter. Put `app`, `goal`, and any layer pinning in metadata_json.
Calculator arithmetic should use `force_path: "hotkey"` and a deterministic
`hotkey_script`; Cursor/VS Code tasks should include `electron_debugging_port`; canvas
fixture tasks should use `app: "browser"` and `force_path: "vision"`.
