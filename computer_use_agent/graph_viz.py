...
def _memory_hits_for_ui(store: SessionStore) -> tuple[list[dict[str, str]], int]:
    """Compact, deduplicated memory hits for the graph sidebar."""
    all_raw = store.load_memory_hits()
    total = len(all_raw)
    seen: set[str] = set()
    ...
    for raw in all_raw:
        if not isinstance(raw, dict):
            continue
        desc = str(raw.get("descriptor") or "memory").strip()
        ...
        key = f"{desc.lower()}|{preview[:120].lower()}"
        if key in seen:
            continue
        seen.add(key)
...
def _format_action_summary(action: Any, index: int) -> str:
    if isinstance(action, str):
        text = action.strip()
        return f"{index}. {text[:120]}" if text else f"{index}. action"
    if not isinstance(action, dict):
        return f"{index}. {action!r}"
    tool = action.get("tool") or action.get("action") or action.get("type") or "action"
    args = action.get("args") or action.get("arguments") or {}
    bits: list[str] = []
    if isinstance(args, dict):
        for key in ("name", "text", "key", "element_index", "action", "url"):
            if args.get(key) is not None:
                bits.append(f"{key}={args[key]!r}")
    suffix = (" " + " ".join(bits[:3])) if bits else ""
    return f"{index}. {tool}{suffix}"


def _parse_node_output(raw: Any) -> dict[str, Any]:
    if isinstance(raw, dict):
        return raw
    if not raw:
        return {}
...
def _node_actions_summary(skill: str, st: Any) -> list[str]:
    """Short action lines for the DAG node detail panel."""
    if st is None or not getattr(st, "output", None):
        return []
    out = _parse_node_output(st.output)
    if not out:
        preview = _preview_text(st.output)
        return [preview] if preview and preview not in ("(running…)",) else []
    ...
    if skill == "computer":
        ...
        if result is not None and len(str(result)) <= 120:
            lines.append(f"result={result}")
        elif out.get("result_preview"):
            lines.append(f"result={out['result_preview']}")
        actions = _flatten_output_actions(out.get("actions") or [])
        if actions:
            lines.append(f"{len(actions)} logged action(s)")
            for i, action in enumerate(actions[:6]):
                lines.append(_format_action_summary(action, i + 1))
        traj = out.get("trajectory_dir")
        if traj:
            lines.append(f"trajectory recorded")
    elif skill == "browser":
        steps = out.get("steps") or out.get("actions") or []
        if isinstance(steps, list) and steps:
            lines.append(f"{len(steps)} browser step(s)")
            for i, step in enumerate(steps[:6]):
                lines.append(_format_action_summary(step, i + 1))
        elif out.get("final_answer"):
            lines.append(f"answer={str(out['final_answer'])[:100]}")
    elif skill == "planner":
        nodes = out.get("nodes")
        if isinstance(nodes, list) and nodes:
            skills = [str(n.get("skill") or "?") for n in nodes if isinstance(n, dict)]
            if skills:
                lines.append("plan: " + " → ".join(skills[:8]))
    elif skill == "formatter":
        text = out.get("text") or out.get("markdown") or out.get("result")
        if text:
            lines.append(f"output={str(text)[:120]}")
    else:
        preview = _preview_text(out)
        if preview:
            lines.append(preview[:160])
    return lines[:8]
...
def graph_viz_payload(session_id: str) -> dict[str, Any]:
    ...
    for nid, data in graph.nodes(data=True):
        ...
        if nid in states:
            actions_summary = _node_actions_summary(skill, states[nid])
        nodes.append(
            {
                "id": nid,
                "label": f"{skill}\n{label}",
    ...
    hits, hits_total = _memory_hits_for_ui(store)
