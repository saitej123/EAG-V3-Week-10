...
_COMPUTER_SETUP_MARKERS = (
    "windows desktop automation",
    "glibc_2.39",
    "cua_driver_win_bin",
    "cua-driver not found",
    "failed to start cua-driver serve",
    "daemon did not start in time",
    "linux cua-driver cannot run",
)

_COMPUTER_INTERACTION_MARKERS = (
    "not in cache",
    "click failed",
    "element_index",
    "interaction_failed",
...
def is_computer_environment_error(error_text: str) -> bool:
    """True only for missing/misconfigured driver — not runtime click/input failures."""
    lower = (error_text or "").lower()
    if any(marker in lower for marker in _COMPUTER_INTERACTION_MARKERS):
        return False
    return any(marker in lower for marker in _COMPUTER_SETUP_MARKERS)
...
def plan_recovery(
    *,
    ...
    lower = (error_text or "").lower()
    if failed_skill == "computer" and is_computer_environment_error(error_text):
        return RecoveryDecision(
            action="skip",
            reason="computer_environment",
            note=error_text,
