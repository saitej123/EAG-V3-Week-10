...
def _build_submission_outline(spec: dict[str, Any]) -> list[dict[str, Any]]:
    """Ordered submission sections for UI assignment cards."""
...
def browser_queries_payload() -> dict[str, Any]:
    """Assignment corpus for Computer Use UI and /api/queries/dag."""
    spec = load_assignment_spec()
    queries = [enrich_assignment_query(q) for q in spec.get("queries", [])]
    outline = _build_submission_outline(spec)
    design = list(spec.get("design_queries", []))
    groups = [
        {
            "part": row["part"],
            "label": row["title"],
    ...
    return {
        ...
        "computer_findings": spec.get("computer_findings", {}),
        "computer_reference_runs": spec.get("computer_reference_runs", []),
        "groups": groups,
        "outline": outline,
        "browser_only": False,
    }
...
def assignment_payload() -> dict[str, Any]:
    """Alias for the UI assignment corpus."""
...
DESIGN_DEFERRALS: list[DesignDeferral] = [
    DesignDeferral(
        ...
        where="computer_use_agent/memory.py — vector-first read()",
    ...
    DesignDeferral(
        ...
        where="computer_use_agent/indexing.py — _chunk_text(), index_document_path()",
    ...
    DesignDeferral(
        ...
        where="computer_use_agent/memory.py — _load_faiss_from_disk()",
        forward_phase="Future release",
    ...
    DesignDeferral(
        ...
        where="computer_use_agent/llm_env.py — gemini_embed_model(), try_embed_text()",
        forward_phase="Future release",
