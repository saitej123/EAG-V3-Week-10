...
class DagAgent:
    ...
    @staticmethod
    def _is_computer_environment_error(error: Exception) -> bool:
        from .recovery import is_computer_environment_error

        return is_computer_environment_error(str(error))

    @staticmethod
    def _computer_environment_answer(error: Exception) -> str:
        return (
            "## Computer driver setup needed\n\n"
            f"{error}\n\n"
            "This is a Windows + WSL2 setup issue, not a browser fallback. "
            "Install `cua-driver` on Windows or set `CUA_DRIVER_WIN_BIN` to the Windows `.exe` path, "
            "then run the computer task again."
        )
    ...
    async def run(self, user_query: str, session_id: str | None = None) -> None:
        ...
        try:
            ...
            if self._is_computer_environment_error(e):
                log_final_answer(self._computer_environment_answer(e))
                logger.info("[agent] RUN_COMPLETE reason=computer_environment")
                return
            raise
        finally:
            try:
